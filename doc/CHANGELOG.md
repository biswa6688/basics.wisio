...
- Real reference photos, one per lesson, added across all 8 topics via a new `TopicPhoto` component — sourced exclusively from Wikimedia Commons with verified licenses (2 public domain: abacus, ENIAC; 6 CC-BY-SA/GPL with visible attribution captions), resized to web-friendly JPEGs. Glossary terms (`GlossaryTerm`) also added for jargon across all 8 lessons (bit, cache, packet, context switch, transistor, microprocessor, signal, servers, instructions). See [PHOTO_CREDITS.md](./PHOTO_CREDITS.md) for full sourcing.
