...
| CSS bundle | ~26 KB | ~5.6 KB |
| JS bundle | ~420 KB | ~132 KB |

Single JS bundle, no code splitting yet — the whole app (all 6 lessons, all illustrations) loads on first visit. For the current app size this is still fast in absolute terms (~132 KB gzipped is well within a good LCP budget on a typical connection), but it's the first thing to address if the app grows past its current 6 topics.
...
- [ ] Route-based code splitting (`React.lazy` per topic page) once the bundle grows meaningfully — not yet worth the complexity at 6 topics sharing most of their component code
